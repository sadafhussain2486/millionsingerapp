...
class ApiController extends Controller
{
    ...
    public function alertenglish($index=null)
    {
        $alertenglish=array(
            ...
            '410'=>'Token Not Match',
            '411'=>'Email Is Empty',
            '412'=>'Pass All Parameter',
            '413'=>'Email Not Change',
            '414'=>'OTP not verify',
            '415'=>'Account is Disable By Administration',
            '416'=>'Error Found',
    ...
    public function alertchinese($index=null)
    {
        $alertchinese=array(
            '200'=>'成功',
            '202'=>'未能儲存',
    ...
    public function actionCreateuser()
    {
        ...
        if(Yii::$app->getRequest()->method==="POST")
        {
            ...
            { 
                if(!empty($data["username"]) && !empty($data["device_id"]) && !empty($data["device_type"]) && !empty($data["register_id"]) && !empty($data["facebook_id"]) && !empty($data["gender"]) && !empty($data["nick_name"]) && !empty($data["image"]))
                {                        
                    $userdata=$model->find()->where(['username' => $data['username'], 'role' => 2, 'status' => 1])
                                ->limit(1)
                                ->orderBy(['id'=>SORT_ASC])
                                ->one(); 
                    if(!empty($userdata))           //ALREADY EXIST (ONLY LOGIN)
                    {
                    ...
                    else                            //NEW USER (REGISTER THEN LOGIN)
                    {
                        $data["regid"]= $regid;       
                        $data['otpverify']=1;
                        $data['status']=1;
    ...
    {           
        ...
        if(Yii::$app->getRequest()->method==="POST")
        {
            ...
            {   
                if(isset($data["register_id"]) && !empty($data["register_id"]) &&  isset($data["register_by"]) && !empty($data["register_by"]) && isset($data["device_type"]) && !empty($data["device_type"]))
                {
                    $userdata=$model->find()->where(['username' => $data['username'], 'role' => 2, 'status' => 1])
                                    ->limit(1)
                                    ->orderBy(['id'=>SORT_ASC])
                ...
                if(!empty($getid))
                {
                    ...
                    {                        
                        if(!empty($data["group_id"]) && isset($data["group_id"]))
                        {
                            ...
                            $openingbal=$model6->find()->where(['id'=>$data["group_id"]])->one();
                            $catdata=$model->find()->where(['user_id'=>$getid["id"],'account'=>$data["group_id"],'status' => 1])
                                            ->andWhere($datefilter)
                                            ->orderBy($sort)
                                            ->all();  
                            //record budget calculate part
                            $incexpdata=$model->find()->where(['user_id'=>$getid["id"],'account'=>$data["group_id"],'recordbudget'=>1,'status' => 1])
                                            ->andWhere($datefilter)
                                            ->orderBy($sort)
                                            ->all();
                        }
                        ...
                        { 
                            //OPENING BALANCE INDIVDUAL
                            $openingbal=$model2->find()->where(['id'=>$getid["id"]])->one();                           
                            $catdata=$model->find()->where(['user_id'=>$getid["id"],'account'=>0,'status' => 1])
                                            ->andWhere($datefilter)
                                            ->orderBy($sort)
                                            ->all();          
                            //record budget calculate part
                            $incexpdata=$model->find()->where(['user_id'=>$getid["id"],'account'=>0,'recordbudget'=>1,'status' => 1])
                                            ->andWhere($datefilter)
                                            ->orderBy($sort)
                                            ->all();           
                        }
                    ...
                    { 
                        foreach ($catdata as $value) 
                        {
                            ...
                            if($data["type"]==$value['type'])
                            {
                                $cat[]=array(
                                    ...
                                    'repeat' => $value['repeat'],
                                    'repitition_period' => $value['repitition_period'],
                                    'budget' => ($value['recordbudget']==1)?true:false,
                                    'status' => $value['status'],
                                    'created_date'=>$value['created_date'],
                                    'modify_date'=>$value['modify_date'],
                                );  
                        ...
                        //Select Budget OF MONTH
                        //$selectbudgetdata=$model5->find()->where(['DATE_FORMAT(created_date, "%Y-%m")' => $year."-".$month,'user_id'=>$getid["id"],'group_id'=>$data["group_id"],'status' => 1])->one();
                        $commandamtexp = $connection->createCommand("SELECT sum(amount) FROM category_user where DATE_FORMAT(created_date,'%Y-%m')='".$year."-".$month."' AND user_id='".$getid["id"]."' AND group_id='".$data["group_id"]."' AND status=1");
                        $selectbudgetdata= $commandamtexp->queryAll();
                        $totalbudget=(!empty($selectbudgetdata))?round($selectbudgetdata[0]['sum(amount)'],0):0;

                        foreach ($incexpdata as $incexpvalue) 
                        {                            
                            //,'category_id'=>$incexpvalue['category_id']
                            $totalincome+=($incexpvalue['type']==1)?$incexpvalue['amount']:0;
                            $totalexpense+=($incexpvalue['type']==2)?$incexpvalue['amount']:0;
                            //$totalbudget+=(!empty($selectbudgetdata))?$selectbudgetdata[0]['sum(amount)']:0;
                        }
                    }
                    else
                    {
                    ...
                    $totalnetworth=(!empty($openingbal["opening_balance"]))?round($openingbal["opening_balance"],2):0;
                    $targetamt=round($target["suggest_target"],2);
                    //$spentamt=round(($target["suggest_target"]-$totalexpense),2);
                    $spentamt=round($totalexpense,2);
                    //$remainamt=round(($targetamt-$spentamt),2);
                    $remainamt=round(($totalbudget-$totalexpense),2);
            ...
            if(!empty($data['token']) && isset($data['token']))
            {
                ...
                if(!empty($getid))
                {
                    ...
                    if(isset($data["mydate"]) && !empty($data["mydate"]))
                    {
                        $year = date("Y",strtotime($data["mydate"]));
                        $month = date("m",strtotime($data["mydate"]));
                        if(empty($data["category_id"]))             //WITHOUT CATEGORY
                        {                            
                            //SELECT FROM AMOUNT TABLE
                            $csumdata=$model->find()->where(['type'=>2,'DATE_FORMAT(selectdate, "%Y-%m")' => $year."-".$month,'user_id' => $getid['id'],'account' => $data["group_id"],'recordbudget'=>1])->groupBy(['category_id'])->all();
                            $categoryid='';
                            $rc=0;
                            if(!empty($csumdata))
                            { 
                                ...
                                foreach ($csumdata as $value) 
                                {
                                    ...
                                    //GET BUDGET OF PARTICULAR CATEGORY FROM CATEGORY_USER TABLE
                                    $budgetamount=$model5->find()->where(['DATE_FORMAT(created_date, "%Y-%m")' => $year."-".$month,'user_id' => $getid['id'],'group_id' => $data["group_id"],'category_id'=>$value["category_id"]])->one(); 

                                    if($language==0)
                                    {
                                        $catname=$model6->categoryname($value['category_id']);
                                    }
                                    else
                                    {   
                                        $catname=$model6->categoryname($value['category_id'],"category_name_c");
                                    }
                                    $cat[$rc]=array(
                                        'id' => $value['id'],
                                        'type'=>'EXPENSE',
                                        ...
                                        'expense_amount' => (!empty($sum))?round($sum,2):0,
                                        'budget'=>(!empty($budgetamount["amount"]))?round($budgetamount["amount"],2):0,                                        
                                    );
                                    $rc++;
                                    $totaltarget1+=$sum;
                                    $totalsumoftarget1+=(!empty($budgetamount['amount']))?$budgetamount['amount']:0;
                            ...
                            //echo $categoryid;
                            ######---ONLY BUDGET--########
                            //SELECT FROM CATEGORY USER TABLE 
                            /*$catuserdata=$model5->find()->where(['DATE_FORMAT(created_date, "%Y-%m")' => $year."-".$month,'user_id' => $getid['id'],'group_id' => $data["group_id"]])
                            ->andWhere(['NOT IN','category_id',[$categoryid]])
                            ->groupBy(['id'=>SORT_ASC])
                            ->all(); */
                            if(!empty($categoryid))
                            {                            
                                $budgetcat = $connection->createCommand("SELECT * FROM category_user where DATE_FORMAT(created_date,'%Y-%m')='".$year."-".$month."' AND user_id='".$getid['id']."' AND group_id='".$data["group_id"]."' AND status=1 AND category_id NOT IN (".$categoryid.")");                            
                                $catuserdata = $budgetcat->queryAll();
                                //print_r($catuserdata);
                                if(!empty($catuserdata))
                                { 
                                    $totaltarget2=0;
                                    $totalsumoftarget2=0;   
                                    foreach ($catuserdata as $catusvalue) 
                                    {     
                                        $catexpdata=$model3->find()->where(['id' => $catusvalue['category_id']])->one(); 
                                        if($language==0)
                                        {
                                            $catname=$catexpdata['category_name'];
                                        }
                                        else
                                        {   
                                            $catname=$catexpdata['category_name_c'];
                                        } 
                                        $cat[$rc]=array(
                                            'id' => $catusvalue['id'],
                                            'type'=>'EXPENSE',
                                            'user_id' => $catusvalue['user_id'],
                                            'category_id' => $catusvalue['category_id'],
                                            'category_name' => ($catname!=null)?$catname:'',
                                            'category_icon' => ($catexpdata['category_icon']!=null && $catexpdata['category_icon']!='a')?$catexpdata['category_icon']:Yii::$app->mycomponent->Siteurl().Yii::$app->request->baseUrl.'/upload/category/dafault.png',
                                            'expense_amount' => '0',
                                            'budget'=>(!empty($catusvalue["amount"]))?round($catusvalue["amount"],2):0,
                                        );
                                        $rc++;
                                        $totaltarget2+=$sum2;
                                        $totalsumoftarget2+=(!empty($catusvalue['amount']))?$catusvalue['amount']:0;

                                    }
                                }
                            }
                            if(!empty($catuserdata) || !empty($csumdata))
                            {
                                $totaltarget=($totaltarget1+$totaltarget2);
                                $totalsumoftarget=($totalsumoftarget1+$totalsumoftarget2);
                                $response['error']=false;
                                $response['statuscode']=200;
                                $response['msg']="Success";
                        ...
                        {
                            $catdata=$model->find()->where(['DATE_FORMAT(selectdate, "%Y-%m")' => $year."-".$month,'user_id'=>$getid["id"],'category_id'=>$data["category_id"],'type' => 2,'account' => $data["group_id"],'status' => 1])
                                ->orderBy(['id'=>SORT_DESC])
                                ->all();
                            ...
                            {                           
                                ...
                                {   
                                    $cat[]=array(
                                        ...
                                        'selectdate' => $value['selectdate'],                                        
                                        'payment_detail' => $value['payment_detail'],
                                        'expense_amount' => round($value['amount'],2),
                                        'account' => $value['account'],
                                        'note' => $value['note'],
                                        'bill_image' => $value['bill_image'],
                                        'repeat' => $value['repeat'],
                                ...
                                $response['budget']=(!empty($csumdata['amount']))?round($csumdata['amount'],2):0;
                                $response['category_name']=($cdata["category_name"]!=null)?$cdata["category_name"]:'';
                            ...
                            {                                
                                ...
                                $response['msg']=$returnmessage[$response['statuscode']];
                                $response['total']=0;
                                $response['budget']=(!empty($csumdata['amount']))?$csumdata['amount']:0;
                            } 
                        }                         
                    }
                    else
        ...
        {   
            ...
            if(!empty($data['token']) && isset($data['token']) && isset($data["request"]) && !empty($data["request"]))
            {
                ...
                if(!empty($getid))
                {
                    ...
                    {
                        $monthname=array('01'=>'一月','02'=>'二月','03'=>'三月','04'=>'四月','05'=>'五月','06'=>'六月','07'=>'七月','08'=>'八月','09'=>'九月','10'=>'十月','11'=>'十一月','12'=>'十二月');   
                    }
                    $currentyear=date('Y');
                    $i=1;
                    //$j=1;
