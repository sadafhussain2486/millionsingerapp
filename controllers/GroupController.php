...
class GroupController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        {
            $amountdata=$model1->find()->where(['status' => 1, 'type' => 1, 'account'=>$id,'DATE_FORMAT(selectdate,"%Y-%m")'=>$monthyear."-".$month])
                                ->orderBy(['selectdate'=>SORT_DESC])
                                ->all();

            $expensedata=$model1->find()->where(['status' => 1, 'type' => 2,'account'=>$id,'DATE_FORMAT(selectdate,"%Y-%m")'=>$monthyear."-".$month])
                                ->orderBy(['selectdate'=>SORT_DESC])
                                ->all(); 
        }
        else
        ...
        {
            //Cash In (Income)
            $commandamtindi = $connection->createCommand("SELECT sum(amount) FROM amount where DATE_FORMAT(selectdate,'%Y-%m')='".trim($cashyear."-".$cashinmonth,'-')."' AND type=1 and account!=0 and status=1 and recordbudget=1 and account=".$id."");
            $analysiscashin['cashin'][$i++] = $commandamtindi->queryAll();

            //Cash Out (Expense)
            $commandamtexp = $connection->createCommand("SELECT sum(amount) FROM amount where DATE_FORMAT(selectdate,'%Y-%m')='".trim($cashyear."-".$cashinmonth,'-')."' AND type=2 and account!=0 and status=1 and recordbudget=1 and account=".$id."");
        ...
        ##################---NEW BUDGET----------#######################
        $newbudget["withoutbudget"]=$model1->find()->where(['type'=>2,'DATE_FORMAT(selectdate, "%Y-%m")' => $monthyear."-".$analsmonth,'account' => $groupdata['id'],'recordbudget'=>1])->groupBy(['category_id'])->all();
        $newbudget["withbudget"]=$model2->find()->where(['group_id' => $groupdata['id'],'DATE_FORMAT(created_date,"%Y-%m")'=>$monthyear."-".$analsmonth,'status'=>'1'])->all();
      
        //print_r($newbudgetdata);
        //$analcategory=$ucatdata;

        ##################---NEW BUDGET END------#######################
        $ucatdata=$model2->find()->where(['group_id'=>$groupdata['id'],'DATE_FORMAT(created_date,"%Y-%m")'=>$monthyear."-".$analsmonth,'status'=>'1'])->all();
        $analcategory=$ucatdata;
        ...
        foreach ($analcategory as $analcatval) 
        {
            ...
            $commandamtexp = $connection->createCommand("SELECT sum(amount) FROM amount where DATE_FORMAT(selectdate,'%Y-%m')='".$cashyear."-".$analsmonth."' AND type=2 and account!=0 and status=1 and recordbudget=1 and account=".$id." and category_id=".$analcatval["category_id"]."");
        ...
        //Analysis Total Sum    
        $commandamttotexp = $connection->createCommand("SELECT sum(amount) FROM amount where DATE_FORMAT(selectdate,'%Y-%m')='".$cashyear."-".$analsmonth."' AND type=2 and account!=0 and status=1 and recordbudget=1 and account=".$id."");
        ...
        $catdata=['amount'=>$amountdata,'expense'=>$expensedata,'user'=>$userdata,'newbudgetdata'=>$newbudget,'categoryuser'=>$ucatdata,'targetuserdata'=>$targetdata,'currentmonth'=>$checkcurrentmntdata,'groupdata'=>$groupdata,'noticedata'=>$noticedata,'cashflow'=>$analysiscashin,'anaycatamt'=>$analysiscatexp,'anaycattotalamt'=>$totalanacatexp,'netspent'=>$netspent,'totalnetspent'=>$totalnetspent,'stringpercnet'=>$stringpercnet];
