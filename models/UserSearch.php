...
class UserSearch extends User
{
    ...
    public function rules()
    {
        return [
            ...
            [['password', 'register_id', 'fb_id', 'image', 'number', 'otp_code', 'device_id', 'nick_name', 'occupation', 'gender', 'opening_balance', 'created_date', 'last_update_date'], 'safe'],
        ]; //, 'device_type' 'age'
    }
...
//        }        
    ...
    public function validateUpdate($data=null)
    {
        ...
        else if(!isset($data['occupation']) || empty($data['occupation']))
        {  
            $error['error']=true;
            $error['statuscode']=317;
            $error['msg']="Please enter the occupation.";
        }
...
//        }
        else if(!isset($data['opening_balance']) || empty($data['opening_balance']))
        {  
            $error['error']=true;
            $error['statuscode']=319;
            $error['msg']="Please enter the opening balance.";
        }
        else
        {
            $error['error']=false;
...
//        }
    ...
    public function isExistBytoken($token)
    {
        if(!empty($token))
        {
            $data = $this->find()->where(['token' => $token])->one();
            //print_r($data);
            if(!empty($data))
            { 
                if(!empty($data->id))
